...
inline xchar* xstr_data(xstr_t* xs)
{
    return xs->data;
}

inline xchar xstr_front(const xstr_t* xs)
{
    return xs->data[0];
}

inline xchar xstr_back(const xstr_t* xs)
{
    return xs->data[xs->size - 1];
}

inline int xstr_empty(const xstr_t* xs)
{
    return xs->size == 0;
}

inline size_t xstr_size(const xstr_t* xs)
{
    return xs->size;
}

inline void xstr_clear(xstr_t* xs)
{
    xs->size = 0;
    xs->data[0] = (xchar)'\0';
}
